...
statNC_interval <- function(
  ...
  for (i in 1:iterations) {
    ...
    emgf <- nonparametric_estimator(
      arrivals = f, conflevel = conflevel,
      blimit = blimit
    )
    inverse_backlog[i] <- statNC_optimize_ibl_theta(
      viol_prob = viol_prob,
...
plot_distribution <- function(computed_dist, stat_mean, stat_lower, stat_upper,
  ...
  pz <- rep(0, length(bl))
  labels <- data.frame(
    y = c(0.2, 0.4), x = c(stat_fail_mean, stat_mean),
    label = c(
      round(stat_fail_mean, digits = 0),
  ...
  q <- ggplot(frame, aes(x = backlog, y = perc)) +
    ...
    geom_vline(
      xintercept = c(stat_fail_lower), colour = "black",
      linetype = "dotted"
    ) +
    geom_vline(
      xintercept = c(stat_fail_upper), colour = "black",
      linetype = "dotted"
    ) +
    ...
    geom_label(aes(
      x = stat_fail_lower - 0.30 * maximum, y = 0.7,
      label = "StatNC (LRD)"
    ), fill = "white", size = 5) +
    geom_segment(aes(
    ...
    geom_segment(aes(
      x = stat_fail_lower - 0.03 * maximum, y = 0.7,
